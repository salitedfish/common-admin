...
import {
  ...
  reAuditResList,
  EditType,
} from "../goodsListManager/goodsListManagerStore";
            ...
            NButton,
            {
              ...
              onClick: () => {
                const dialogInfo = dialog.warning({
                  title: "移出回收站",
                  content: `确认将${goods.goodsName}移出回收站吗？`,
                  positiveText: "确认",
                  onPositiveClick: async () => {
                    dialogInfo.loading = true;
                    const res = await updateGoodsRecycleState({ goodsId: goods.goodsId, goodsRecycleState: AntinomyTypes.NOT });
                    if (res) {
                      await getList();
                      commonNotify("success", "商品移出成功");
                    }
...
const { totalPage, getList, searchParam, list, listXWidth, listYHeight, searching, submitSearch } = useListPage(getListRequest, createColumns, {
  params: {
    goodsRecycleState: AntinomyTypes.YES,
  },
});
