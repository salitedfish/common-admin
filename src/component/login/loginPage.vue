        ...
        <section class="action-form">
          <section class="form">
            ...
            <div class="captcha-box" v-show="loginType === LoginType.CAPTCHA">
              ...
              <n-button type="primary" @click="getCaptchaHandler" :loading="captchaGetState">获取验证码</n-button>
            </div>
          </section>
          <n-button class="login-btn" @click="switchLoginTypeHandler" block>切换登录方式</n-button>
          <n-button class="login-btn" type="primary" block @click="loginHandler" :loading="loginState">登录</n-button>
...
import { passwordLegal } from "@/util";
...
const notifyHandler = (message: string, type: NotificationType) => {
  notify[type]({
    content: message,
    duration: 2000,
...
const loginHandler = async () => {
  ...
  if (loginType.value === LoginType.PASSWORD) {
    if (!formData.password) {
      notifyHandler("密码不能为空!", "warning");
      return;
    }
    if (!passwordLegal(formData.password)) {
      notifyHandler("密码格式不正确！", "warning");
      return;
    }
