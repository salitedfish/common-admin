...
// 网络请求
import { getOrderList as getOrderListRequest, orderOffLineComfirm as orderOffLineComfirmRequest, orderSyncComfirm as orderSyncComfirmRequest } from "@/request/order";
// store
import { useCommonStore } from "@/store/commonStore";
import { invoiceState, orderStateList, orderTypeList, payChannelList, orderExchangeTypeList, OrderState, PayChannel } from "./orderListManagerStore";
...
const listYHeight = computed(() => {
  return commonStore.pageContentHeight - 228;
});

// 筛选的参数
const searchParam = ref<RequestParam.GetOrderList>({
  page: 1,
  size: 10,
});
// 数据总数
...
const getOrderList = async () => {
  ...
  const res = await getOrderListRequest(searchParam.value);
  if (res) {
...
// 组合请求参数
const submitSearch = (params: OrderListParam) => {
