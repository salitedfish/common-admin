...
// 当前类目indexList
const activeCategoryIndexList = reactive<number[]>([0, 0, 0, 0]);
...
const getCategoryTree = async () => {
  const res = await getCategoryTreeRequest(CategoryType.GOODS);
  if (res) {
...
const handleUpdate = (index?: number, i?: number) => {
  if (index !== undefined && index >= 0 && i !== undefined && i >= 0) {
    ...
    updateCategoryInfo.type = CategoryType.GOODS;
    updateCategoryInfo.parentId = parentId;
  }
